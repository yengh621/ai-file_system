...
/* 自然语言交互（GLM 集成）*/
static int execute_nlp_command_line(char *exec_cmd) {
    char cmd_name[64] = "";
    char arg1[256] = "";
    char arg2[256] = "";
    char arg3[256] = "";
    int n = sscanf(exec_cmd, "%63s %255s %255s %255[^\n]", cmd_name, arg1, arg2, arg3);
    ...
    if (n >= 1) {
        if (strcmp(cmd_name, "create") == 0 && n >= 2) {
            ...
            return 1;
        } else if (strcmp(cmd_name, "grant") == 0 && n >= 4) {
            grant(arg1, arg2, atoi(arg3));
            return 1;
        } else if (strcmp(cmd_name, "link") == 0 && n >= 3) {
            link(arg1, arg2);
            return 1;
        } else if (strcmp(cmd_name, "symlink") == 0 && n >= 3) {
            symlink(arg1, arg2);
            return 1;
        } else if (strcmp(cmd_name, "unlink") == 0 && n >= 2) {
            fs_unlink(arg1);
            return 1;
        } else if (strcmp(cmd_name, "readlink") == 0 && n >= 2) {
            char buf[DIRSIZ + 1];
...
void nlp_interact(char *text) {
    ...
    char output[1024];
    char exec_cmds[16][256];
    int exec_count = 0;
    char exec_cmd[256] = "";
    int has_exec = 0;
    
    while (fgets(output, sizeof(output), pipe) != NULL) {
        if (strncmp(output, "EXEC:", 5) == 0 && exec_count < 16) {
            strncpy(exec_cmds[exec_count], output + 5, sizeof(exec_cmds[exec_count]) - 1);
            exec_cmds[exec_count][sizeof(exec_cmds[exec_count]) - 1] = '\0';
            char *newline = strchr(exec_cmds[exec_count], '\n');
            if (newline) *newline = '\0';
            if (!has_exec) {
                strncpy(exec_cmd, exec_cmds[exec_count], sizeof(exec_cmd) - 1);
    ...
    if (has_exec && strlen(exec_cmd) > 0) {
        ...
        for (int i = 1; i < exec_count; i++) {
            if (strcmp(exec_cmds[i], "unknown") == 0 || strlen(exec_cmds[i]) == 0) {
                continue;
            }
            printf("\n========================================\n");
            printf("姝ｅ湪鎵ц [%d/%d]: %s\n", i + 1, exec_count, exec_cmds[i]);
            printf("========================================\n\n");
            if (!execute_nlp_command_line(exec_cmds[i])) {
                printf("鏈敮鎸佺殑鍛戒护: %s\n", exec_cmds[i]);
            }
        }
    } else {
